                ...
                <div class="card card-4">
                    <div class="card-header  " style="direction: rtl">
                        <div class="row justify-content-between  text-right">
                            <div class="col-xl-4 m-3">
                                <h4 class=" text-right"><i class="fal fa-file-chart-line fa-fw"></i>گزارش فعالیت های اجرایی پژوهشی</h4>
                            </div>
                        </div><!-- /card-tools -->
                    ...
                    <div class="card-body  p-4">
                        ...
                        <div class="row justify-content-start  mt-3">
                            ...
                            <div  class="col-lg-4  mt-3" >
                                <div class="form-group mb-3 text-right">
                                    <label class="blue text-right  text-rtl">نوع فعالیت اجرایی :</label>
                                    <!-- @change="searchit" -->
                                    <Select2  class="form-control select2-form-control non-modal-select2" id="research_activity_Type_id"
                                              v-model="research_activity_Type_id"
                                              :options="research_activity_types"

                                              :settings="{theme: 'bootstrap4', placeholder: 'انتخاب نوع فعالیت اجرایی', width: '100%' ,multiple: true}">
                                    </Select2>
                        ...
                        <div class="row justify-content-center  mt-2">
                            ...
                            <download-excel
                                ...
                                :fetch="getExcel"
                                :meta="json_meta"
                                :exportFields="researchActivity_fields"

                                worksheet = "My Worksheet"
                                name    = "researchActivities.xls">
                        ...
                        <div v-if="showReport" class="card-body table-responsive p-0 table-sm my-5">
                            ...
                            <table class="table  table-striped table-hover text-right">
                                ...
                                <tr v-for="(researchActivity, index) in researchActivities" :key="researchActivity.id">
                                    ...
                                    <td>{{ researchActivity.faculty }}</td>
                                    <td>{{ researchActivity.department }}</td>
                                    <td >{{researchActivity.authorities | truncate(50)}} </td>
                                    <td >{{researchActivity.start_date | myDate}} </td>
                                    <td >{{researchActivity.end_date | myDate}} </td>
                                    ...
                                    <td colspan="2">{{ researchActivity.created_at | myDate  }}</td>
    ...
    export default {
        ...
        data(){
            return{
                ...
                researchActivity_fields:{
                    'عنوان سمت یا مسئولیت' : 'title',
                    'نام ثبت کننده' : 'Author_name',
                    'نوع مسئولیت' : 'research_activity_type_name',
                    'مرجع تایید کننده' : 'authorities',
                    'نام دانشکده' : 'faculty',
                    'نام گروه' : 'department',
        ...
        methods:{
            ...
            resetForm(){
                ...
                this.start_date='';
                this.end_date='';
                this.research_activity_Type_id=[];
                this.faculty_id=[];
                this.department_id=[];
                this.status=[];
                this.perPage=5;
            ...
            async  getExcel(page = 1){
                ...
                const response = await axios.post('/api/researchActivityReport?order=' + sortOrder + '&term_id=' + this.term_id + '&research_activity_Type_id=' + this.research_activity_Type_id +
            ...
            getResults(page = 1) {
                ...
                axios.post('/api/researchActivityReport?order=' + sortOrder + '&term_id=' + this.term_id + '&research_activity_Type_id=' + this.research_activity_Type_id  +
                    '&start_date=' + this.start_date + '&end_date=' + this.end_date + '&status=' + this.status +'&faculty_id=' + this.faculty_id
            ...
            getResearchActivityRelation(){
                axios.get('/api/researchActivityReportRelation')
                    .then(response => {
        ...
        created() {
            this.$parent.pageName = 'گزارش فعالیت های اجرایی پژوهشی';
            Fire.$on('searching', () => {
                let page = 1;
