<template>
    <div>
        <div class="container-fluid text-rtl text-right my-5">
            <div class="card card-4 my-5">
                <div class="card-header justify-content-around d-flex " style="direction: rtl">
                    <div class="col">
                        <h3 class=" text-right"><i class="fal fa-dollar-sign fa-fw"></i> اطلاعات بودجه جذب شده</h3>
                    </div>
                ...
                <div class="card-body  px-0">
                    <table class="table table-hover table-striped">
                        ...
                        <tr>
                            <th class="font-18"><i class="fal fa-dollar-sign fa-fw"></i> اطلاعات بودجه </th>
                        ...
                        <tr>
                            ...
                            <td v-if="checkList">
                                <p-check :checked="checkListForm.list && checkListForm.list.includes('عنوان بودجه')"
                                         @change.native="onChange('عنوان بودجه', $event)"
                        ...
                        <tr>
                            ...
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('نوع بودجه')"
                                    @change.native="onChange('نوع بودجه', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            ...
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('میزان بودجه')"
                                    @change.native="onChange('میزان بودجه', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            ...
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('تاریخ جذب بودجه')"
                                    @change.native="onChange('تاریخ جذب بودجه', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-for="(file, index) of grant.files" class="mr-3 ">
                                    <div v-if="checkFileType(file)" class="btn-group text-ltr" role="group" aria-label="Basic example">
                                         <a  class="btn btn-lg btn-info" :href="'../files/grants/'+file.name"><i class="fal fa-download fa-fw"></i>  </a>
                                         <a  class="btn btn-lg btn-info" :href="'../files/grants/'+file.name">ضمیمه {{index+1|faDigit}}  <i class="fal fa-file-archive fa-fw"></i></a>
                                    </div>
                                    <div v-else class="btn-group text-ltr" role="group" aria-label="Basic example">
                                      <a  :href="'../files/grants/'+file.name" class="btn btn-lg btn-info"><i class="fal fa-download"></i></a>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-if="grant.status == '0'"  class="teal"><i class="fal fa-question"></i>  {{'بررسی نشده' }}</span>
                                <span v-else-if="grant.status == '1'"  class="green"><i class="fal fa-check"></i>  {{'تایید شده' }}</span>
                                <span v-else-if="grant.status == '2'"  class="orange"><i class="far fa-exclamation-triangle"></i>  {{'عدم تایید موقت' }}</span>
                                <span v-else-if="grant.status == '3'"  class="red"><i class="fal fa-times"></i>  {{'عدم تایید قطعی' }}</span>
                                    ...
                                    <div v-if="checkListForm.status == 1" class="form-inline">
                                        <div class="form-group w-25 text-right">
                                            ...
                                            <input type="number" :min="grant.minScore" :max="grant.maxScore" name="score"
                                                   ...
                                                   v-validate="{min_value:grant.minScore,max_value:grant.maxScore}"
            ...
            <div class="row align-content-center">
                <div class="fixed-bottom mx-auto d-md-flex bg-white   justify-content-center py-2">
                    <!-- grantEditModal checkListSubmit toggleCheckList checkListHistory-->
                    <button v-if="grant.status != 3 && grant.status != 1" @click="grantEditModal" class="btn btn-lg mx-1 btn-secondary">ویرایش  گرنت</button>
        ...
        <div  v-if="$gate.isAdmin()||$gate.isAuthor()" class="modal  fade" id="grantEditModal" tabindex="-1" role="dialog" aria-labelledby="addNewLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" >
                        <form-wizard
                            ...
                            <!--  -->
                            <tab-content title="اطلاعات گرنت" :before-change="grantValidation"  icon="far fa-users-class">
...
<script>
    ...
    methods:{
        ...
        onComplete: function(){
            ...
            this.form.submit('post', `/api/grantUpdate/${this.grant.id}`, {
        ...
        checkListSubmit(){
            ...
            this.checkListForm.post('/api/grantCheckList')
                .then((response) => {
                    this.checkListItems.unshift(response.data.checkListItem);
                    this.grant.status = this.checkListForm.status;
                    this.grant.score = response.data.score;
        ...
        prepareCheckList(){
            if(this.grant.status != '0') {
                if (this.checkListItems.length > 0 && this.grant.status != '1') {
                    this.checkListForm.list = $.map(this.checkListItems[0].list, function (value, index) {
                        return [value];
                    });
                }
                if (this.checkListItems.length > 0){
                    this.checkListForm.comment = this.checkListItems[0].comment;
                    this.checkListForm.status = this.grant.status;
                }
            }
            this.checkListForm.score = this.grant.score;
        },
        toggleCheckList(){
        ...
        goback(){
            this.$router.push({path:'/grants'});
        },
        // shows grantEditModal
        grantEditModal(){
            $('#grantEditModal').modal('show');
        },
        // prepares the edit form data
        editFormPrepare(){
            this.form.fill(this.grant);
            this.form.fileChangeType ='';
            this.form.files =[];
        },
        pdfModal(name){
            this.pdfFileName = '../files/grants/'+name+'#page=1';
            $('#pdfModal').modal('show');
        },
        ...
        // on page load gets Grant data based on the received it
        getGrantData(id){
            axios.get(`/api/grant/${id}`)
                .then(response => {
                    this.grant = response.data.data;
                    this.checkListItems = response.data.data.checkList;
                    this.term_form.term_id = this.grant.term_id;
        ...
        termChangeSubmit(){
            ...
            this.term_form.post('/api/termChange')
                .then((res)=>{
                    loader1.hide();
                    this.grant.term_name = res.data.term_name;
                    this.grant.term_id = res.data.term_id;
