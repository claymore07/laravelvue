<template>
    <div>
        <div class="container-fluid text-rtl text-right my-5">
            <div class="card card-4 my-5">
                <div class="card-header justify-content-around d-flex " style="direction: rtl">
                    <div class="col">
                        <h3 class=" text-right">اطلاعات جایزه</h3>
                    </div>
                ...
                <div class="card-body  px-0">
                    <table class="table table-hover table-striped">
                        ...
                        <tr>
                            <th class="font-18"><i class="fal fa-file-edit fa-fw"></i>اطلاعات جایزه</th>
                            <th v-if="checkList">موارد ناقص</th>
                        </tr>
                        </thead>
                        <tbody v-if="reward.id">
                        <tr>
                            <td class="font-16">
                                <span class="blue">عنوان جشنواره، رقابت یا مراسم:</span>
                                <span class="mr-3">{{reward.name}}</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('عنوان جشنواره')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check :checked="checkListForm.list && checkListForm.list.includes('عنوان جشنواره')"
                                         @change.native="onChange('عنوان جشنواره', $event)"
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue">نام برنده جایزه:</span>
                                <span class="mr-3">{{reward.Author_name}}</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('نام برنده جایزه')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check :checked="checkListForm.list && checkListForm.list.includes('نام برنده جایزه')"
                                         @change.native="onChange('نام برنده جایزه', $event)"
                                         type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue">عنوان جایزه:</span>
                                <span class="mr-3">{{reward.title}}</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('عنوان جایزه')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check :checked="checkListForm.list && checkListForm.list.includes('عنوان جایزه')"
                                         @change.native="onChange('عنوان جایزه', $event)"
                                         type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">نوع جشنواره:</span>
                                <span class="mr-3">{{reward.type}}</span>

                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('نوع جشنواره')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('نوع جشنواره')"
                                    @change.native="onChange('نوع جشنواره', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">رتبه کسب شده:</span>
                                <span v-if="reward.place == 1" class="mr-3">رتبه اول</span>
                                <span v-else-if="reward.place == 2" class="mr-3">رتبه دوم</span>
                                <span v-else-if="reward.place == 3" class="mr-3">رتبه سوم</span>

                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('رتبه کسب شده')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('رتبه کسب شده')"
                                    @change.native="onChange('رتبه کسب شده', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">دوره برگزاری:</span>
                                <span class="mr-3 persian-num">{{reward.period}}</span>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">تاریخ دریافت جایزه:</span>
                                <span class="mr-3 "> {{reward.holding_date | myDate}} هجری شمسی</span>
                                <span class="mr-3 "> {{reward.holding_date | myDateEN}} میلادی</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('تاریخ دریافت جایزه')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('تاریخ دریافت جایزه')"
                                    @change.native="onChange('تاریخ دریافت جایزه', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-for="(file, index) of reward.files" class="mr-3 ">
                                    <div v-if="checkFileType(file)" class="btn-group text-ltr" role="group" aria-label="Basic example">
                                         <a  class="btn btn-lg btn-info" :href="'../files/rewards/'+file.name"><i class="fal fa-download fa-fw"></i>  </a>
                                         <a  class="btn btn-lg btn-info" :href="'../files/rewards/'+file.name">ضمیمه {{index+1|faDigit}}  <i class="fal fa-file-archive fa-fw"></i></a>
                                    </div>
                                    <div v-else class="btn-group text-ltr" role="group" aria-label="Basic example">
                                      <a  :href="'../files/rewards/'+file.name" class="btn btn-lg btn-info"><i class="fal fa-download"></i></a>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-show="!TermChange"  class="mr-3 persian-num">{{reward.term_name}}</span>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-if="reward.status == '0'"  class="teal"><i class="fal fa-question"></i>  {{'بررسی نشده' }}</span>
                                <span v-else-if="reward.status == '1'"  class="green"><i class="fal fa-check"></i>  {{'تایید شده' }}</span>
                                <span v-else-if="reward.status == '2'"  class="orange"><i class="far fa-exclamation-triangle"></i>  {{'عدم تایید موقت' }}</span>
                                <span v-else-if="reward.status == '3'"  class="red"><i class="fal fa-times"></i>  {{'عدم تایید قطعی' }}</span>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-if="reward.status != '1'"  ><i class="fal fa-question"></i>  {{'امتیازی ثبت نشده' }}</span>
                                <span v-else class="persian-num">  {{reward.score  }}</span>
                            </td>
                            <td v-if="checkList">
                            </td>
                        </tr>
                                ...
                                <form  data-vv-scope="checkListForm">
                                    <div  class="form-inline">
                                        <div class="form-group w-25 text-right">
                                            ...
                                            <input type="number" :min="reward.minScore" :max="reward.maxScore" name="score"
                                                   placeholder=""
                                                   :class="[( errors.has('checkListForm.score') || checkListForm.errors.has('score') ? 'is-invalid': ''  )]"
                                                   v-validate="{min_value:reward.minScore,max_value:reward.maxScore}"
                                                   class="form-control w-50" v-model="checkListForm.score" >
                                        </div>
                                        <div class="form-group mb-2">
                                            <span class="persian-num">توجه: امتیاز این آیتم با توجه به بخشنامه در بازه {{reward.minScore}} و {{reward.maxScore}} در نظر گرفته میشود.</span>
                                        </div>
                                    </div>
        ...
        <div   class="modal  fade" id="rewardEditModal" tabindex="-1" role="dialog" aria-labelledby="addNewLabel" aria-hidden="true">
    ...
    export default {
        ...
        methods:{
            ...
            checkListSubmit(){
                ...
                this.checkListForm.post('/api/rewardCheckList')
                    .then((response) => {
            ...
            prepareCheckList(){
                if(this.reward.status != '0') {
                    if (this.checkListItems.length > 0 && this.reward.status != '1') {
                        this.checkListForm.list = $.map(this.checkListItems[0].list, function (value, index) {
                            return [value];
                        });
                    }
                ...
                this.checkListForm.score = this.reward.score;
            },
            toggleCheckList(){
            ...
            // shows rewardEditModal
            rewardEditModal(){
                $('#rewardEditModal').modal('show');
            },
            // prepares the edit form data
            editFormPrepare(){
                this.form.fill(this.reward);
                this.form.fileChangeType = '';
                this.form.files =[];
            },
            pdfModal(name){
                this.pdfFileName = '../files/rewards/'+name+'#page=1';
                $('#pdfModal').modal('show');
            },
