<template>
    <div>
        <div class="container-fluid text-rtl text-right my-5">
            <div class="card card-4 my-5">
                <div class="card-header justify-content-around d-flex " style="direction: rtl">
                    <div class="col">
                        <h3 class=" text-right"><i class="fal fa-user-secret fa-fw"></i>اطلاعات فعالیت اجرایی پژوهشی</h3>
                    </div>
                ...
                <div class="card-body  px-0">
                    <table class="table table-hover table-striped">
                        ...
                        <tr>
                            <th class="font-18"><i class="fal fa-dollar-sign fa-fw"></i> اطلاعات فعالیت اجرایی پژوهشی </th>
                            <th v-if="checkList">موارد ناقص</th>
                        </tr>
                        </thead>
                        <tbody v-if="researchActivity.id">
                        <tr>
                            <td class="font-16">
                                <span class="blue">عنوان سمت یا مسئولیت:</span>
                                <span class="mr-3">{{researchActivity.title}}</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('عنوان سمت یا مسئولیت')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check :checked="checkListForm.list && checkListForm.list.includes('عنوان سمت یا مسئولیت')"
                                         @change.native="onChange('عنوان سمت یا مسئولیت', $event)"
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue">نام ثبت کننده:</span>
                                <span class="mr-3">{{researchActivity.Author_name}}</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('نام ثبت کننده')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check :checked="checkListForm.list && checkListForm.list.includes('نام ثبت کننده')"
                                         @change.native="onChange('نام ثبت کننده', $event)"
                                         type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">نوع سمت یا مسئولیت:</span>
                                <span class="mr-3">{{researchActivity.research_activity_types_name}}</span>

                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('نوع سمت یا مسئولیت')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('نوع سمت یا مسئولیت')"
                                    @change.native="onChange('نوع سمت یا مسئولیت', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">نام مرجع تایید کننده:</span>
                                <span class="mr-3">{{researchActivity.authorities}}</span>

                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('نام مرجع تایید کننده')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('نام مرجع تایید کننده')"
                                    @change.native="onChange('نام مرجع تایید کننده', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">تاریخ شروع فعالیت:</span>
                                <span class="mr-3 "> {{researchActivity.start_date | myDate}} هجری شمسی</span>
                                <span class="mr-3 "> {{researchActivity.start_date | myDateEN}} میلادی</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('تاریخ شروع فعالیت')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('تاریخ شروع فعالیت')"
                                    @change.native="onChange('تاریخ شروع فعالیت', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                <span class="blue ">تاریخ اتمام فعالیت:</span>
                                <span class="mr-3 "> {{researchActivity.end_date | myDate}} هجری شمسی</span>
                                <span class="mr-3 "> {{researchActivity.end_date | myDateEN}} میلادی</span>
                                <span class="red float-left font-20" v-if="checkListForm.list && checkListForm.list.includes('تاریخ اتمام فعالیت')" title="عدم تایید"><i @click="checkListHistory"  class="fa fa-times-circle fe-pulse-w-pause "></i></span>
                            </td>
                            <td v-if="checkList">
                                <p-check
                                    :checked="checkListForm.list && checkListForm.list.includes('تاریخ اتمام فعالیت')"
                                    @change.native="onChange('تاریخ اتمام فعالیت', $event)"
                                    type="checkbox" class="p-icon p-curve p-pulse p-bigger text-ltr" color="info-o">
                                    <i slot="extra" class="icon far fa-check"></i>
                                </p-check>
                            </td>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-for="(file, index) of researchActivity.files" class="mr-3 ">
                                    <div v-if="checkFileType(file)" class="btn-group text-ltr" role="group" aria-label="Basic example">
                                         <a  class="btn btn-lg btn-info" :href="'../files/research_activity/'+file.name"><i class="fal fa-download fa-fw"></i>  </a>
                                         <a  class="btn btn-lg btn-info" :href="'../files/research_activity/'+file.name">ضمیمه {{index+1|faDigit}}  <i class="fal fa-file-archive fa-fw"></i></a>
                                    </div>
                                    <div v-else class="btn-group text-ltr" role="group" aria-label="Basic example">
                                      <a  :href="'../files/research_activity/'+file.name" class="btn btn-lg btn-info"><i class="fal fa-download"></i></a>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span  v-show="!TermChange" class="mr-3 persian-num">{{researchActivity.term_name}}</span>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-if="researchActivity.status == '0'"  class="teal"><i class="fal fa-question"></i>  {{'بررسی نشده' }}</span>
                                <span v-else-if="researchActivity.status == '1'"  class="green"><i class="fal fa-check"></i>  {{'تایید شده' }}</span>
                                <span v-else-if="researchActivity.status == '2'"  class="orange"><i class="far fa-exclamation-triangle"></i>  {{'عدم تایید موقت' }}</span>
                                <span v-else-if="researchActivity.status == '3'"  class="red"><i class="fal fa-times"></i>  {{'عدم تایید قطعی' }}</span>
                        ...
                        <tr>
                            <td class="font-16">
                                ...
                                <span v-if="researchActivity.status != '1'"  ><i class="fal fa-question"></i>  {{'امتیازی ثبت نشده' }}</span>
                                <span v-else  class="persian-num">  {{researchActivity.score  }}</span>
                            </td>
                            <td v-if="checkList">
                            </td>
                        </tr>
                                ...
                                <form  data-vv-scope="checkListForm">
                                    <div  class="form-inline">
                                        <div class="form-group w-25 text-right">
                                            ...
                                            <input type="number" :min="researchActivity.minScore" :max="researchActivity.maxScore" name="score"
                                                   placeholder=""
                                                   :class="[( errors.has('checkListForm.score') || checkListForm.errors.has('score') ? 'is-invalid': ''  )]"
                                                   v-validate="{min_value:researchActivity.minScore,max_value:researchActivity.maxScore}"
                                                   class="form-control w-50" v-model="checkListForm.score" >
                                        </div>
                                        <div class="form-group mb-2">
                                            <span class="persian-num">توجه: امتیاز این آیتم با توجه به بخشنامه در بازه {{researchActivity.minScore}} و {{researchActivity.maxScore}} در نظر گرفته میشود.</span>
                                        </div>
                                    </div>
            ...
            <div class="row align-content-center">
                <div class="fixed-bottom mx-auto d-md-flex bg-white   justify-content-center py-2">
                    <!-- researchActivityEditModal checkListSubmit toggleCheckList checkListHistory-->
                    <button v-if="researchActivity.status != 3 && researchActivity.status != 1" @click="researchActivityEditModal" class="btn btn-lg mx-1 btn-secondary">ویرایش  فعالیت</button>
        ...
        <div class="modal  fade" id="researchActivityEditModal" tabindex="-1" role="dialog" aria-labelledby="addNewLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl  modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel"><i
                            class="far fa-users-class fa-fw"></i> ویرایش اطلاعات گرنت</h5>
                    ...
                    <div class="modal-body" >
                        <form-wizard
                            ...
                            <!--  -->
                            <tab-content title="اطلاعات فعالیت اجرایی پژوهشی" :before-change="researchActivityValidation"  icon="far fa-users-class">
                                <form @submit.prevent="createResearchActivity()" @keydown="form.onKeydown($event)" @change="form.onKeydown($event)" data-vv-scope="form" id="Form">
    ...
    export default {
        ...
        methods:{
            ...
            onComplete: function(){
                ...
                this.form.submit('post', `/api/researchActivityUpdate/${this.researchActivity.id}`, {
                    ...
                    this.researchActivity = response.data.data;
                    this.checkListItems = response.data.data.checkList;
                    this.editFormPrepare();
                    // UI modal Hide, progress bar finish , show the toase
                    $('#researchActivityEditModal').modal('hide');
                    this.successToast('اطلاعات فعالیت اجرایی پژوهشی با موفقیت ویرایش شد.');
                    this.successToast('اطلاعات فعالیت اجرایی پژوهشی با موفقیت ویرایش شد.');
                ...
                if(this.checkListItems.length > 1){
                    this.checkListForm.delete('/api/deleteCheckListItem/' + id).then(() => {
                        ...
                        if(this.checkListForm.list.length > 0){
                            ...
                            this.researchActivity.status = 2;
                        }
            ...
            checkListSubmit(){
                ...
                this.checkListForm.post('/api/researchActivityCheckList')
                    .then((response) => {
                        this.checkListItems.unshift(response.data.checkListItem);
                        this.researchActivity.status = this.checkListForm.status;
                        this.researchActivity.score = response.data.score;
            ...
            prepareCheckList(){
                if(this.researchActivity.status != '0') {
                    if (this.checkListItems.length > 0 && this.researchActivity.status != '1') {
                        this.checkListForm.list = $.map(this.checkListItems[0].list, function (value, index) {
                            return [value];
                        });
                    }
                    if (this.checkListItems.length > 0){
                        this.checkListForm.comment = this.checkListItems[0].comment;
                        this.checkListForm.status = this.researchActivity.status;
                    }
                }
                this.checkListForm.score = this.researchActivity.score;
            },
            toggleCheckList(){
            ...
            // shows researchActivityEditModal
            researchActivityEditModal(){
                $('#researchActivityEditModal').modal('show');
            },
            // prepares the edit form data
            editFormPrepare(){
                this.form.fill(this.researchActivity);
                this.form.fileChangeType ='';
                this.form.files =[];
            },
            pdfModal(name){
                this.pdfFileName = '../files/research_activity/'+name+'#page=1';
                $('#pdfModal').modal('show');
            },
            ...
            // on page load gets research_activity data based on the received it
            getResearchActivityData(id){
                axios.get(`/api/researchActivity/${id}`)
                    .then(response => {
                        this.researchActivity = response.data.data;
                        this.checkListItems = response.data.data.checkList;
                        this.term_form.term_id = this.researchActivity.term_id;
            ...
            termChangeSubmit(){
                ...
                this.term_form.post('/api/termChange')
                    .then((res)=>{
                        loader1.hide();
                        this.researchActivity.term_name = res.data.term_name;
                        this.researchActivity.term_id = res.data.term_id;
        ...
        created(){
            ...
            this.getResearchActivityData(this.id);
